...
def marshall_datasets(
    dataset_name_pattern: str, country_pattern: str, hdx_site: str = "stage"
):
    if dataset_name_pattern.lower() != "all":
        create_datasets_in_hdx(
            dataset_name_pattern,
            country_filter=country_pattern,
            hdx_site=hdx_site,
            use_legacy=True,
        )
    else:
...
def create_or_fetch_base_dataset(
    ...
    if dataset is not None and not force_create:
        is_new = False
        LOGGER.info(
            f"Fetching `{dataset_label}` from hdx_site: `{Configuration.read().hdx_site}`"
        )
    else:
        LOGGER.info(
            f"Creating `{dataset_label}` from `{dataset_attributes['dataset_template']}`"
        )
...
def find_resource_filepath(
    ...
    if spreadsheet_directory is None:
        spreadsheet_directory = os.path.join(
            os.path.dirname(__file__), "output-spreadsheets"
        )
...
def configure_hdx_connection(hdx_site: str = "stage"):
    try:
        ...
        Configuration.create(
            user_agent_config_yaml=os.path.join(
                os.path.expanduser("~"), ".useragents.yaml"
            ),
            user_agent_lookup="hdx-scraper-insecurity-insight",
            hdx_site=hdx_site,
        )
        LOGGER.info(
            f"Authenticated to HDX site at {Configuration.read().get_hdx_site_url()}"
        )
        ...
        LOGGER.info(
            "Configuration already exists when trying to create in `create_datasets.py`"
        )
