...
def generate_schema(dataset_name: str, api_fields_basis: bool = False) -> str:
    ...
    if "overview" in dataset_name:
        print(
            "Overview files should be updated manually in schema-overview.csv",
            flush=True,
        )
        return status
    ...
    print(
        f"{'':<2}   {'Spreadsheet column':<50},{'HXL tag':<50}, {'api_field':<50}",
        flush=True,
    )
    for i, column in enumerate(columns):
        ...
        print(
            f"{i:<2}.  {column[0]:<50.50},{column[1]:<50.50}, {api_field:<50.50}",
            flush=True,
        )
...
def find_corresponding_api_field(
    dataset_name: str, api_fields: list, column: str
) -> str:
